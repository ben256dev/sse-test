...
#define VECTORS_BYTES VECTOR_COUNT * sizeof(v3)
    ...
    for (int i = 0; i < (VECTOR_COUNT * 3 / V3_SUB); i++)
    {
        ...
        __m128 v_3 = _mm_loadu_ps(&vectors[i][8]);
        __m128 v_4 = _mm_loadu_ps(&vectors[i][12]);
        __m128 v_5 = _mm_loadu_ps(&vectors[i][16]);
        __m128 v_6 = _mm_loadu_ps(&vectors[i][20]);
        __m128 v_7 = _mm_loadu_ps(&vectors[i][24]);
        __m128 v_8 = _mm_loadu_ps(&vectors[i][28]);
        ...
        _mm_storeu_ps(&vectors[i][0],  v_res_1);
        _mm_storeu_ps(&vectors[i][4],  v_res_2);
        _mm_storeu_ps(&vectors[i][8],  v_res_3);
        _mm_storeu_ps(&vectors[i][12], v_res_4);
    }
    clock_t end = clock();
